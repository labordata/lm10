...
INSERT INTO counterparty_organization (
    activity_id, city, organization, "po_box,_bldg,_room_no,_if_any",
    state, street, "zip_code_+_4"
)
SELECT
    a.id, raw.city, raw.organization, raw."po_box,_bldg,_room_no,_if_any",
    raw.state, raw.street, raw."zip_code_+_4"
FROM raw_counterparty_organization raw
JOIN (
    SELECT id, rptId,
           ROW_NUMBER() OVER (PARTITION BY rptId ORDER BY id) - 1 AS ordinal
      FROM activity
     WHERE rptId IN (SELECT DISTINCT CAST(rptId AS INTEGER) FROM raw_counterparty_organization)
