...
DELETE FROM reportable_activity
WHERE
  rptId IN (SELECT DISTINCT rptId FROM raw_reportable_activity);

INSERT INTO reportable_activity(rptId, answer, code, n_responses, question)
SELECT rptId, answer, code, n_responses, question
FROM raw_reportable_activity;
